...
async def fetch_semantic_scholar(query: str, limit: int = 5) -> List[Dict]:
    ...
    async with httpx.AsyncClient() as client:
        r = await client.get(SEMANTIC_SCHOLAR_URL, params=params, headers=headers)
        r.raise_for_status()
...
async def fetch_reliefweb(query: str, limit: int = 5) -> List[Dict]:
    ...
    async with httpx.AsyncClient() as client:
        ...
        r.raise_for_status()
        data = r.json().get("data", [])
        print(f"  ✓ ReliefWeb completed")
        return data


async def fetch_who_iris(query: str, limit: int = 5) -> List[Dict]:
    print("Fetching from WHO IRIS...")
    # WHO IRIS uses OAI-PMH protocol, so we need to construct the request
    params = {
        "verb": "ListRecords",
...
async def aggregate(query: str) -> List[Dict]:
    ...
    # Print results count from each source
    print(f"\nResults fetched per source:")
    print(f"  Semantic Scholar: {len(ss_data)} results")
    print(f"  OpenAlex: {len(oa_data)} results")
    print(f"  ReliefWeb: {len(rw_data)} results")
    print(f"  WHO IRIS: {len(who_data)} results")
